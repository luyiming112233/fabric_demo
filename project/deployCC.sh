...
go mod vendor
popd
echo Finished Go Mod dependencies
...
packageChaincode() {
  ...
  setGlobals $ORG
  set -x
  peer lifecycle chaincode package ${CC_NAME}.tar.gz --path ${CC_SRC_PATH} --lang golang --label ${CC_NAME} >&log.txt
  res=$?
  set +x
  cat log.txt
...
chaincodeInvokeInit() {
  ...
  # it using the "-o" option
  set -x
  peer chaincode invoke -o localhost:7050 --ordererTLSHostnameOverride orderer.example.com --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA -C $CHANNEL_NAME -n ${CC_NAME} $PEER_CONN_PARMS --isInit -c '{"Args":[]}' >&log.txt
  res=$?
  set +x
  cat log.txt
